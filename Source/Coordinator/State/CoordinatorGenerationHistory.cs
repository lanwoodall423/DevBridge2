...
internal sealed partial class CoordinatorState
{
        ...
        out GenerationManifest manifest)
    {
        ...
        try
        {
            ...
            if (File.Exists(path))
            {
                ...
                manifest = JsonSerializer.Deserialize<GenerationManifest>(serialized, Program.JsonOptions);
            ...
            try
            {
                AtomicCreateGenerationFile(path, JsonSerializer.Serialize(manifest, Program.JsonOptions));
            }
        ...
        out string errorCode, out string error)
    {
        ...
        try
        {
            ...
            envelope = JsonSerializer.Deserialize<GenerationHistoryEnvelope>(serialized, Program.JsonOptions);
    ...
    private bool TryWriteGenerationHistoryLocked(GenerationHistoryEnvelope envelope)
    {
        try
        {
            AtomicWriteGenerationFile(generationHistoryPath,
                JsonSerializer.Serialize(envelope, Program.JsonOptions));
            return true;
        }
        catch (IOException)
        {
        ...
        GenerationHistoryView view)
    {
        ...
        try
        {
            ...
            RequireSchemaContract(serialized, DevBridgeSchemaVersions.GenerationManifest,
                DevBridgeSchemaVersions.GenerationManifestContract, requireRecords: false);
            GenerationManifest manifest = JsonSerializer.Deserialize<GenerationManifest>(serialized, Program.JsonOptions);
    ...
    private static void AtomicWriteGenerationFile(string path, string contents)
    {
    ...
    private static void AtomicCreateGenerationFile(string path, string contents)
    {
        string directory = Path.GetDirectoryName(path) ?? Directory.GetCurrentDirectory();
        Directory.CreateDirectory(directory);
        string temporary = path + ".tmp-" + Guid.NewGuid().ToString("N");
